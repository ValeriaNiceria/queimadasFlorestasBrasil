server <- function(input, output, server) {
  ...
  queimadas_estado <- reactive({
    if (input$select_estado_ano != "Todos os anos") {
      queimadas <-
        dados %>% 
        filter(state == input$select_estado,
               year == input$select_estado_ano) %>% 
        group_by(month) %>% 
        summarise(total_queimadas = sum(number))
  ...
  output$graf_estado_ano_queimadas <- renderHighchart({
    
    if (input$select_estado_ano != "Todos os anos") {
      queimadas_estado() %>% 
        hchart(type = "line", name = "Total queimadas durante o mês", hcaes(x = month, y = total_queimadas)) %>% 
        hc_xAxis(title = list(text = "Mês")) %>% 
        hc_yAxis(title = list(text = "Total de queimadas")) %>% 
        hc_colors(c("#9b0000"))
    } else {
